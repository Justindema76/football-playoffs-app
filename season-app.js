(() => {
  ...
  const state={
    view:'team',pos:'ALL',q:'',week:Number.isFinite(REQUESTED_WEEK)&&REQUESTED_WEEK>=1&&REQUESTED_WEEK<=18?REQUESTED_WEEK:1,
    playerSort:'PROJ',players:[],intel:[],weather:[],owner:[],suggestions:[],roster:[],
    leagueTeams:[],leagueRosters:[],leagueMatchups:[],weekStats:[],playerPool:[],leagueWeek:1,errors:[]
  };
  ...
  function weeklyGroups(rows){
    ...
    for(const row of rows||[]){
      ...
      if(row.yahoo_player_key){const k=String(row.yahoo_player_key);if(!yahoo.has(k))yahoo.set(k,[]);yahoo.get(k).push({...row,week:w})}
      if(row.player_key){const k=String(row.player_key);if(!player.has(k))player.set(k,[]);player.get(k).push({...row,week:w})}
    }
    return {yahoo,player};
  }
  ...
  function buildMyRoster(teams,rosters){
    ...
    if(!me)return [];
    return (rosters||[]).filter(r=>r.league_team_id===me.id&&r.active!==false).map((r,index)=>({
      ...
      active:true,updated_at:r.last_synced_at||r.updated_at||null
  ...
  function rosterPlayer(row){
    ...
    if(yahoo){const exact=state.players.find(p=>String(p.yahoo_player_key||'')===yahoo);if(exact)return exact}
  ...
  function teamWeekRows(teamId,week){
    ...
    if(rows.length)return rows.map((r,index)=>({
      player_key:r.player_key||r.yahoo_player_key||r.yahoo_player_name,yahoo_player_key:r.yahoo_player_key||null,
      player_name:r.yahoo_player_name||'Player',team:r.nfl_team||null,position:r.position||null,
      roster_slot:r.roster_slot||r.position||'',lineup_group:r.is_starter===false?'BENCH':'STARTER',
      display_order:Number.isFinite(Number(r.lineup_order))?Number(r.lineup_order):index+1,_weekly:r
    ...
    if(!team)return [];
    return state.leagueRosters.filter(r=>r.league_team_id===teamId&&r.active!==false).map((r,index)=>({
      player_key:r.player_key||r.yahoo_player_key||r.yahoo_player_name,yahoo_player_key:r.yahoo_player_key||null,
      player_name:r.yahoo_player_name||'Player',team:r.nfl_team||null,position:r.position||null,
      roster_slot:r.roster_slot||r.position||'',lineup_group:isBenchSlot(r.roster_slot)?'BENCH':'STARTER',
      display_order:Number.isFinite(Number(r.lineup_order))?Number(r.lineup_order):index+1,_weekly:weeklyForRow(r,week)
    })).sort((a,b)=>(a.display_order-b.display_order)||slotWeight(a.roster_slot)-slotWeight(b.roster_slot));
  }
  ...
  function setWeek(week){
    ...
    state.week=w;state.leagueWeek=w;
    const u=new URL(location.href);u.searchParams.set('week',String(w));history.replaceState(null,'',`${u.pathname}${u.search}#${state.view}`);
    render();
  }
  ...
  function bindWeekButtons(){ $('content').querySelectorAll('[data-review-week]').forEach(btn=>btn.onclick=()=>setWeek(btn.dataset.reviewWeek)); }
  ...
  function renderTeam(){
    ...
    const roster=source.filter(rosterMatches),starters=roster.filter(x=>x.lineup_group==='STARTER'),bench=roster.filter(x=>x.lineup_group==='BENCH');
    const rb=roster.filter(x=>x.position==='RB').length,wr=roster.filter(x=>x.position==='WR').length,te=roster.filter(x=>x.position==='TE').length,qb=roster.filter(x=>x.position==='QB').length;
  ...
  function rosterSection(title,rows){return `<section class="roster-section"><div class="roster-section-head"><b>${title}</b><span>${rows.length} players</span></div><div class="roster-grid">${rows.map(rosterCard).join('')||'<div class="empty">No players.</div>'}</div></section>`}
  function rosterCard(row){
    const p=rosterPlayer(row),tags=p?playersApi.allTags(p).filter(t=>t!=='TARGET'):[],lower=tags.map(t=>String(t).toUpperCase());
    const latest=p?intelApi.latest(p.intel_items||[]):null,context=p?.planner_reason||latest?.recommendation||latest?.what_changed||'No material roster change currently logged.';
    const w=row._weekly||weeklyFor(p,state.week)||weeklyForRow(row,state.week),statusClass=lower.includes('INJURY')?'injury':lower.includes('MONITOR')?'monitor':'';
  ...
  function myMatchup(week){return state.leagueMatchups.find(m=>Number(m.week)===Number(week)&&(String(m.team_a_yahoo_key)===String(MY_TEAM_KEY)||String(m.team_b_yahoo_key)===String(MY_TEAM_KEY)))||null}
  function starterRowsFor(team,week){return team?teamWeekRows(team.id,week).filter(x=>x.lineup_group==='STARTER').sort((a,b)=>(a.display_order-b.display_order)||slotWeight(a.roster_slot)-slotWeight(b.roster_slot)):[]}
  function matchupPlayer(row){
    if(!row)return '<div class="matchup-player empty-side">—</div>';
    const p=rosterPlayer(row),w=row._weekly||weeklyFor(p,state.week)||weeklyForRow(row,state.week),action=intelApi.latest(p?.intel_items||[])?.action;
  ...
  function renderMatchup(){
    ...
    $('pageMeta').textContent=`Week ${state.week} · Yahoo matchup + weekly player scoring`;
    if(!m){$('content').innerHTML=`${weekToolbar()}<div class="empty">No matchup is saved for your team in Week ${state.week} yet.</div>`;bindWeekButtons();return}
    const mineA=String(m.team_a_yahoo_key)===String(MY_TEAM_KEY),myKey=mineA?m.team_a_yahoo_key:m.team_b_yahoo_key,oppKey=mineA?m.team_b_yahoo_key:m.team_a_yahoo_key;
    const my=teamByYahooKey(myKey),opp=teamByYahooKey(oppKey),mine=starterRowsFor(my,state.week),theirs=starterRowsFor(opp,state.week),count=Math.max(mine.length,theirs.length);
    const myPts=mineA?m.team_a_points:m.team_b_points,myProj=mineA?m.team_a_projected:m.team_b_projected,oppPts=mineA?m.team_b_points:m.team_a_points,oppProj=mineA?m.team_b_projected:m.team_a_projected;
  ...
  function visiblePlayers(){return state.players.filter(matches).sort(playerSort)}
  function renderPlayerContext(p){
    const suggestions=p.suggestions||[];let out='';
  ...
  function renderPlayers(){
    ...
    $('pageMeta').textContent=`Week ${state.week} · ${a.length} Yahoo players`;
    $('content').innerHTML=`${weekToolbar()}<div class="player-toolbar"><span class="source-badge">YAHOO MASTER PLAYERS · weekly scoring + ownership + Fantasy Intel</span><label class="player-sort-label">SORT <select id="playerSort" class="player-sort"><option value="PROJ" ${state.playerSort==='PROJ'?'selected':''}>PROJECTED</option><option value="PTS" ${state.playerSort==='PTS'?'selected':''}>ACTUAL POINTS</option><option value="YAHOO" ${state.playerSort==='YAHOO'?'selected':''}>YAHOO RANK</option><option value="ROSTERED" ${state.playerSort==='ROSTERED'?'selected':''}>% ROSTERED</option></select></label></div><div class="list">${a.map(playerCard).join('')||'<div class="empty">No players match.</div>'}</div>`;
    bindWeekButtons();bindTargets();$('playerSort').onchange=e=>{state.playerSort=e.target.value;renderPlayers()};
  }
  function playerCard(p){
    const po=normPos(p.position),ts=playersApi.allTags(p),w=weeklyFor(p,state.week),pool=p.pool_status||{};
    const status=String(pool.availability_status||p.availability_status||'FREE_AGENT').replaceAll('_',' '),owner=pool.owning_team_name||p.owning_team_name||'';
    ...
    const ownership=[status,owner,`Yahoo #${p.yahoo_rank||'—'}`,p.percent_rostered!=null?`${p.percent_rostered}% rostered`:null,p.percent_started!=null?`${p.percent_started}% started`:null,p.waiver_clear_text||null].filter(Boolean).join(' · ');
    return `<article class="player-card ${p.is_rostered||p.user_target?'targeted':''}"><div class="card-top"><span class="pos ${po}">${po}</span><div style="display:flex;gap:7px;align-items:center"><span class="rank">${esc(p.team||'FA')}</span>${action}</div></div><div class="player-name">${esc(p.yahoo_name||p.display_name)}</div><div class="player-meta">${esc(ownership)}</div>${weeklySummary(w)}${ts.length?`<div class="tags">${ts.map(t=>`<span class="tag ${tagClass(t)}">${esc(t)}</span>`).join('')}</div>`:''}${renderPlayerContext(p)}</article>`;
  }
  ...
  async function toggleTarget(key,target){
    const token=localStorage.getItem(APP_KEY);if(!token){openKey();return}
  ...
  function renderWeather(){
    const a=state.weather.filter(matchGeneric);$('pageMeta').textContent=`${a.length} watched games`;
  ...
  function leagueTeamMatches(team,rows){if(!state.q)return true;const q=state.q;if([team.team_name,team.manager_name,team.yahoo_team_key].join(' ').toLowerCase().includes(q))return true;return rows.some(r=>[r.yahoo_player_name,r.nfl_team,r.position,r.roster_slot].join(' ').toLowerCase().includes(q))}
  function scoreText(points,projected){if(num(points)!==null&&num(points)!==0)return fmt(points);if(num(projected)!==null)return `Proj ${fmt(projected)}`;return '—'}
  ...
  function leaguePlayerRow(row){
    const p=rosterPlayer({player_key:row.player_key,yahoo_player_key:row.yahoo_player_key,player_name:row.yahoo_player_name}),tags=p?playersApi.allTags(p).filter(t=>t!=='TARGET').slice(0,3):[],w=weeklyFor(p,state.leagueWeek);
  ...
  function leagueTeamCard(team){
    let rows=leagueRosterFor(team);if(state.q)rows=rows.filter(r=>[team.team_name,team.manager_name,r.yahoo_player_name,r.nfl_team,r.position,r.roster_slot].join(' ').toLowerCase().includes(state.q));
  ...
  function renderLeague(){
    const matchups=state.leagueMatchups||[],weeks=state.leagueWeek==='all'?[...new Set(matchups.map(m=>Number(m.week)).filter(Boolean))].sort((a,b)=>a-b):[state.leagueWeek];
    const ordered=[...state.leagueTeams].sort((a,b)=>(String(b.yahoo_team_key)===String(MY_TEAM_KEY))-(String(a.yahoo_team_key)===String(MY_TEAM_KEY))||Number(a.yahoo_team_key||99)-Number(b.yahoo_team_key||99)),visibleTeams=ordered.filter(team=>leagueTeamMatches(team,leagueRosterFor(team)));
    ...
    const schedule=weeks.map(week=>{const rows=matchups.filter(m=>Number(m.week)===Number(week));return `<div class="league-week-block"><div class="league-week-title">WEEK ${week} · ${rows.length} MATCHUPS</div><div class="league-matchup-grid">${rows.map(leagueMatchupCard).join('')||'<div class="empty">No matchup data for this week.</div>'}</div></div>`}).join('');
  ...
  function renderTagged(){const a=state.players.filter(p=>playersApi.isInjury(p,intelApi)).filter(matches).sort(playerSort);$('pageMeta').textContent=`${a.length} current players`; $('content').innerHTML=`${weekToolbar()}<div class="list">${a.map(playerCard).join('')||'<div class="empty">No current injuries match.</div>'}</div>`;bindWeekButtons();bindTargets()}
  ...
  $('keyButton').onclick=openKey;$('saveKey').onclick=saveKey;document.querySelectorAll('[data-close-key]').forEach(x=>x.onclick=closeKey);
  ...
  const requested=(location.hash||'#team').slice(1).toLowerCase();if(VIEWS.includes(requested))state.view=requested;if(state.view==='intel')state.pos='STARRED';
