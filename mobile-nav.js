(() => {
  ...
  function appHref(view){return onIndex ? `#${view}` : `index.html#${view}`;}
  function closeMenu(){
  ...
  function build(){
    ...
    panel.querySelectorAll('[data-mobile-view]').forEach(link=>link.addEventListener('click',e=>{
      const view=link.dataset.mobileView;
      if(activateView(view)){e.preventDefault();panel.querySelectorAll('a').forEach(a=>a.classList.remove('active'));link.classList.add('active');closeMenu();}
    }));
    if(onIndex && location.hash){const requested=location.hash.slice(1).toLowerCase();setTimeout(()=>activateView(requested),40);}
  }
