(() => {
  ...
  const state={catalog:[],byName:new Map(),sent:new Set(),lastScan:0,observer:null,statusEl:null,lastFound:0};
  ...
  async function loadCatalog(){
    ...
    state.catalog.forEach(p=>{
      [p.yahoo_name,p.display_name].map(norm).filter(Boolean).forEach(n=>state.byName.set(n,p));
    });
  }
  ...
  function looksLikePickContext(text){
    ...
    return /(round|rd\.?)[\s#:.-]*\d+/i.test(t)||/(pick|pk\.?)[\s#:.-]*\d+/i.test(t)||/\b\d{1,2}\.\d{1,2}\b/.test(t)||/selected by|drafted by|was drafted|has drafted/i.test(t);
  }
  ...
  function playerFromShortText(text){
    const n=norm(text);
    if(!n)return null;
    if(state.byName.has(n))return state.byName.get(n);
    if(n.length>90)return null;
    for(const [name,p] of state.byName){
      if(name.length<5)continue;
      if(n===name||n.startsWith(`${name} `)||n.endsWith(` ${name}`))return p;
    }
    return null;
  }
  ...
  function hasPickAncestor(el){
    ...
    for(let depth=0;node&&depth<8;depth++,node=node.parentElement){
      ...
      if(!text)continue;
      if(insideAvailableOnly(text)&&text.length<500)return false;
  ...
  function scanByPlayerElements(found){
    ...
    for(const el of nodes){
      ...
      const p=playerFromShortText(text);
      if(!p)continue;
  ...
  function scanByRows(found){
    ...
    selectors.forEach(sel=>{
      document.querySelectorAll(sel).forEach(el=>{
        ...
        const text=clean(el.innerText||el.textContent||'');
        if(text.length<8||text.length>700||!looksLikePickContext(text)||insideAvailableOnly(text))return;
        const n=norm(text);
        for(const [name,p] of state.byName){
          if(name.length<5)continue;
          if(n.includes(name))found.set(norm(p.player_key),p);
        }
      });
    });
  }
  ...
  async function markDrafted(players){
    ...
    if(!fresh.length){setStatus(`LIVE SYNC · ${state.lastFound} FOUND · ${state.sent.size} GONE`);return;}
  ...
  async function scan(force=false){
    ...
    try{
      ...
      chrome.storage.local.set({lastScan:Date.now(),draftedCount:state.sent.size,foundCount:state.lastFound,lastUrl:location.href,lastError:null});
    }catch(e){
  ...
  function start(){
    ...
    chrome.runtime.onMessage.addListener((msg,_sender,sendResponse)=>{
      if(msg?.type==='SCAN_NOW')scan(true).then(()=>sendResponse({ok:true,count:state.sent.size,found:state.lastFound})).catch(e=>sendResponse({ok:false,error:e.message}));
      else if(msg?.type==='STATUS')sendResponse({ok:true,count:state.sent.size,found:state.lastFound,isDraftPage:isDraftPage(),url:location.href});
      return true;
    });
  }
